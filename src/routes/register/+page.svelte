<script lang="ts">
  ...
  async function handleRegister() {
    ...
    try {
      const response = await api.register(email, password, fullName || undefined);
...
<svelte:head>
  <title>Sign Up - Legal Compliance Checker</title>
</svelte:head>
...
<div class="auth-container">
  <div class="auth-card">
    ...
    <p class="subtitle">Get started with compliance scanning</p>
      ...
      <div class="form-group">
        ...
        {#if password && passwordErrors.length > 0}
          <div class="password-requirements">
            ...
            <ul>
              {#each passwordErrors as requirement}
                <li class="requirement-error">{requirement}</li>
              {/each}
            </ul>
          </div>
        {/if}
